...
class _AddMaterialPageState extends State<AddMaterialPage> {
  ...
  bool _isFetchingResponsibles = true;
  ...
  Future<void> _fetchResponsibles() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('data').doc('lists').get();
      if (doc.exists) {
        setState(() {
          _responsibles.addAll(List<String>.from(doc['suppliers']));
          _isFetchingResponsibles = false;
        });
      }
    } catch (e) {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              children: _responsibles.map((String responsible) {
                return ListTile(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('إضافة مواد', style: TextStyle(fontSize: 20.sp, color: Colors.white)),
        backgroundColor: Colors.black.withOpacity(0.7),
      ),
      body: Stack(
        children: [
                ...
                child: Column(
                  children: [
                    ...
                    if (_isFetchingMaterials)
                      CircularProgressIndicator(color: Colors.orange.withOpacity(0.8)),
                    Card(
                      margin: EdgeInsets.symmetric(horizontal: 40.w, vertical: 8.h),
                      elevation: 2,
                      color: Colors.orange.withOpacity(0.8),
