...
class AddPipePage extends StatefulWidget {
  const AddPipePage({super.key});

  @override
  _AddPipePageState createState() => _AddPipePageState();
}

class _AddPipePageState extends State<AddPipePage> {
  ...
  Future<void> _fetchPipes() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('data').doc('lists').get();
      if (doc.exists) {
        setState(() {
          _products = List<String>.from(doc['pipes']);
          _isFetching = false;
        });
      }
    } catch (e) {
      print('حدثت مشكلة من فضلك تأكد من الانترنت وحاول مجددا');
      setState(() {
        _isFetching = false;
      });
    }
  ...
  void _saveData() async {
    if (!_dataModified) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('البيانات محفوظة بالفعل')),
      );
      return;
    }
    ...
    for (var product in _addedProducts) {
      ...
      if (query.docs.isNotEmpty) {
        ...
        DocumentReference newProductRef = await FirebaseFirestore.instance.collection('pipes').add({
          'product': product['product'],
          'date': product['date'],
          'amount': product['amount'],
        });

        await newProductRef.collection('changes').add({
          'date': product['date'],
          'amount': product['amount'],
          'type': 'increase',
        });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('إضافة مواسير', style: TextStyle(fontSize: 20.sp, color: Colors.white)),
